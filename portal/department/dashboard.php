...
$page_title = 'Dashboard | Department';
ob_start();
?>
                ...
                <div class="card shadow px-3 h-100 d-flex justify-content-center">
                    <div class="d-flex flex-row align-items-center">
                        ...
                        <div class="d-flex flex-column align-items-end w-100 h-100">
                            <?php
                            $stmt = $pdo->prepare("SELECT * FROM documents WHERE is_deleted = 0 AND hidden = 0 AND status = 'Ongoing' AND sender = '$code'");
                            $stmt->execute();
                            $count = $stmt->rowCount();
                            ?>
            ...
            <div class="col-xl-4 col-md-6 col-sm-6 col-12" style="margin-bottom: 30px">
                <div class="card shadow px-3 h-100 d-flex justify-content-center">
                    <div class="d-flex flex-row align-items-center">
                        ...
                        <div class="d-flex flex-column align-items-end w-100 h-100">
                            <?php
                            $stmt = $pdo->prepare("SELECT * FROM documents WHERE is_deleted = 0 AND hidden = 0 AND status = 'Done' AND sender = '$code'");
                            $stmt->execute();
                            $count = $stmt->rowCount();
                            ?>
                            <p style="font-weight: 900; color: #5D87FF;" class="h1"><?= $count ?></p>
...
$(document).ready(function() {
    ...
    var dataTable = $('#table').DataTable({
        ...
        "sScrollXInner": "100%",
        "ajax": {
            url: "../../backend/datatables/department/dashboard",
            type: "POST",
